...
namespace OpenCBS.CoreDomain.Contracts.Savings.CalculateInterests
{
    public class DateCalculationStrategy
    {
        ...
        public static bool DateCalculationMonthly(DateTime lastDate, DateTime pDate, int usageTime = 0)
        {
            return lastDate.Year < pDate.Year || (lastDate.Year == pDate.Year && lastDate.Month < pDate.AddMonths(usageTime).Month);
        }

        public static bool DateCalculationMonthlyAfterUsage(DateTime lastDate, DateTime pDate, int usageTime)
        {
            return lastDate.Year < pDate.Year || (lastDate.Year == pDate.Year && lastDate.Month < pDate.AddMonths(usageTime).Month);
        }
