...
namespace OpenCBS.CoreDomain.Contracts.Savings
{
    public interface ISavingsContract : ICloneable
    {
        ...
        List<SavingEvent> GetCancelableFeeEvent(SavingEvent lastSavingEvent);
        ...
        List<SavingEvent> Withdraw(OCurrency pAmount, DateTime pDate, string pDescription, string pReferenceNumber, User pUser, bool pIsDesactivateFees, Teller teller, PaymentMethod paymentMethod);

        List<SavingEvent> Deposit(OCurrency pAmount, DateTime pDate, string pDescription, string pReferenceNumber, User pUser, bool pIsDesactivateFees,
            bool isPending, bool? isLocal, OSavingsMethods savingsMethod, PaymentMethod paymentMethod, int? pendingEventId, Teller teller);
        ...
        SavingEvent DebitTransfer(ISavingsContract to, OCurrency amount, OCurrency fee, DateTime date, string description, string pReferenceNumber);
        SavingEvent CreditTransfer(ISavingsContract from, OCurrency amount, DateTime date, string description, string pReferenceNumber);
        List<SavingEvent> Transfer(ISavingsContract to, OCurrency amount, OCurrency fee, DateTime date, string description, string pReferenceNumber);
        SavingOverdraftFeeEvent ChargeOverdraftFee(DateTime date, User user, string pReferenceNumber);
