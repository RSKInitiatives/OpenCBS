...
namespace OpenCBS.CoreDomain.Contracts.Savings
{
    ...
    public abstract class SavingsContract : ISavingsContract
    {
        ...
        public OCurrency GetBalance()
        {
            ...
            OCurrency avbalance = GetBalance(TimeProvider.Now) - GetAvailBalance(TimeProvider.Now);
            return avbalance;
        }        

        public bool HasPendingEvents()
        {
            return Events.Any(savingEvent => savingEvent.IsPending && !savingEvent.Deleted);
        }
        /*
        ...
        */
        ...
        public OCurrency GetBalance(DateTime date)
        {
            ...
            foreach (SavingEvent e in Events)
            {
                if ((e is SavingPendingDepositEvent && !e.IsPending) 
                    || 
                    (e is SavingPendingDepositRefusedEvent && !e.IsPending)
                    )
                    continue;

                if ((e is SavingAMFeeEvent && e.IsPending && !e.Deleted))
                    retval += e.Fee;
        ...
        public OCurrency GetAvailBalance(DateTime date)
        {
            ...
            foreach (SavingEvent e in Events)
            {
                //if (!(e is SavingPendingDepositEvent) && !(e is SavingPendingDepositRefusedEvent) && !e.IsPending)
                //    retval += e.Amount;
                if ((e is SavingPendingDepositEvent && e.IsPending)/* || (e is SavingPendingDepositRefusedEvent && e.IsPending)*/)
                    retval += e.Amount;
                else if ((e is SavingAMFeeEvent && e.IsPending && !e.Deleted))
                    retval += e.Fee;                
                else if (e is SavingBlockCompulsarySavingsEvent)
                    retval += e.Amount;
                else if (e is SavingUnblockCompulsorySavingsEvent)
                    retval -= e.Amount;
            }
        ...
        public virtual SavingOverdraftFeeEvent ChargeOverdraftFee(DateTime pDate, User pUser, string referenceNumber)
        {
        ...
        public virtual SavingEvent DebitTransfer(ISavingsContract to, OCurrency amount, OCurrency fee, DateTime date, string description, string pReferenceNumber)
        {
            ...
            e.ReferenceNumber = pReferenceNumber;
        ...
        public virtual SavingEvent CreditTransfer(ISavingsContract from, OCurrency amount, DateTime date, string description, string pReferenceNumber)
        {
            SavingTransferEvent e;
        ...
        public virtual List<SavingEvent> Transfer(ISavingsContract to, OCurrency amount, OCurrency fee, DateTime date, string description, string pReferenceNumber)
        {
            List<SavingEvent> events = new List<SavingEvent>(3);
            events.Add(DebitTransfer(to, amount, fee, date, description, pReferenceNumber));
            events.Add(to.CreditTransfer(this, amount, date, description, pReferenceNumber));
            if (GetBalance() < 0)
            {
                SavingOverdraftFeeEvent e = ChargeOverdraftFee(date, User.CurrentUser, pReferenceNumber);
                events.Add(e);
            }
            return events;
        }
        ...
        public SavingEvent CancelLastEvent()
        {
            ...
            //slycode cancel possible fee events
            List<SavingEvent> feeEvents = GetCancelableFeeEvent(retval);
            foreach(SavingEvent feeEvent in feeEvents)
                CancelEvent(feeEvent);
        ...
        public string GenerateSavingCode(Client client,
                                         int pSavingsCount,
                                         ...
                                         string pBranchCode)
        {
            switch (pCodeTemplate)
            {
                case "BC/YY/PC-PS/CN-ID":
                    {
                        string clientName = (client is Person) ? ((Person)client).LastName : client.Name;
                ...
                case "IC/BC/CS/ID":
                    {
                        string clientCode = client.Id.ToString().PadLeft(5, '0');
                ...
                default:
                    {
                        ...
                        pBranchCode = pBranchCode.Substring(0, Math.Min(pBranchCode.Length, 2)).ToUpper();
                        string clientCode = client.Id.ToString(CultureInfo.InvariantCulture).PadLeft(4, '0');
                        string savingsCount = (pSavingsCount + 1).ToString(CultureInfo.InvariantCulture).PadLeft(2, '0');
                        string productCode = Product.Code.Replace(" ", "");
                        productCode = productCode.Substring(0, Math.Min(productCode.Length, 2)).ToUpper();

                        //Code = string.Format("{0}{1}{2}", clientCode + savingsCount, pBranchCode, productCode);


                        Code = pCodeTemplate
                        .Replace("BC", pBranchCode)
                            //.Replace("DT", client.District.Name.Substring(0, 1 == client.District.Name.Length ? 1 : 2))
                            //.Replace("YY", CreationDate.Year.ToString(CultureInfo.InvariantCulture).Substring(2, 2))
                            //.Replace("yyyy", CreationDate.Year.ToString(CultureInfo.InvariantCulture))
                        .Replace("PC", productCode)
                        .Replace("CID", clientCode)
                        .Replace("AC", savingsCount);
                            //.Replace("CN/ID", clientName.ToUpper()) + "/" + client.Id;
                        break;
                    }
            }
            return Code;
