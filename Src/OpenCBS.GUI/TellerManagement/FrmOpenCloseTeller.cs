...
namespace OpenCBS.GUI.TellerManagement
{
    public partial class FrmOpenCloseTeller : SweetBaseForm
    {
        ...
        private void btnConfirm_Click(object sender, EventArgs e)
        {
            try
            {
                if(String.IsNullOrEmpty(tbAmount.Text))
                    throw new Exception("Teller amount is required");

                OCurrency amount = decimal.Parse(tbAmount.Text);
                if (_isOpen)
                {
                    foreach (Teller teller in ServicesProvider.GetInstance()
                        .GetTellerServices().FindAllNonDeletedTellersOfUser(User.CurrentUser))
                    {
        ...
        private void FrmOpenCloseTeller_Load(object sender, EventArgs e)
        {
