...
namespace OpenCBS.GUI.Tools
{
    public static class Extensions
    {
        ...
        public static void LoadParticularType(this ComboBox comboBox)
        {
            var items = comboBox.Items;
            items.Clear();
            var driverLicense = new DictionaryEntry(OParticularsType.DriverLicense, OParticularsType.DriverLicense.Description());
            var intlPassport = new DictionaryEntry(OParticularsType.IntlPassport, OParticularsType.IntlPassport.Description());
            var votersCard = new DictionaryEntry(OParticularsType.VotersCard, OParticularsType.VotersCard.Description());
            var letterOfIntroduction = new DictionaryEntry(OParticularsType.LetterOfIntroduction, OParticularsType.LetterOfIntroduction.Description());

            items.Add(driverLicense);
            items.Add(intlPassport);
            items.Add(votersCard);
            items.Add(letterOfIntroduction);
            comboBox.SelectedItem = driverLicense;
        }
        ...
        public static void SelectValue(this ComboBox comboBox, int type)
        {
            foreach (DictionaryEntry item in comboBox.Items)
            {
                if ((int)item.Key == type)
                    comboBox.SelectedItem = item;
            }
        }        
