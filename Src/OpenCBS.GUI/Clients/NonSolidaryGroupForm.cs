            ...
            try
            {
                ...
                if (clientServices.ClientIsAPerson(client))
                {
                    ...
                    List<ISavingsContract> savingsContracts =
                        ServicesProvider.GetInstance().GetAccountServices().GetSavingsByClientId(member.Tiers.Id);
        ...
        private void DisplaySavings()
        {
            ...
            foreach (var member in _village.Members)
            {
                if (member.Tiers.Savings.Count > 0)
                {
                    ...
                    List<ISavingsContract> savings = ServicesProvider.GetInstance().GetAccountServices().GetSavingsByClientId(member.Tiers.Id);
            ...
            if (client.Projects != null)
                foreach (var credit in client.Projects.Where(project => project.Credits != null).SelectMany(project => project.Credits))
                    credit.CompulsorySavings =
                        ServicesProvider.GetInstance().GetAccountServices().GetSavingForLoan(credit.Id, true);
