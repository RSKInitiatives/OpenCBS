...
namespace OpenCBS.GUI
{
    public partial class MainView : SweetBaseForm, IMainView
    {
        ...
        private static /*readonly*/ IApplicationController _applicationController;
        ...
        public MainView(IApplicationController applicationController)
        {
            ...
            try
            {
                ...
                _applicationController.Subscribe<EditClientMessage>(this, OnEditClient);
                ...
                var appTimeOut = ServicesProvider.GetInstance().GetGeneralSettings().ApplicationTimeout;                
                logTimer = new TimerPlus();
                logTimer.Interval = appTimeOut;
                logTimer.Elapsed += LogOutTimer_Elapsed;
                base.UserActivity += OnUserActivity;
                logTimer.Start();
            }
        ...
        //slycode
        private void OnEditClient(EditClientMessage message)
        {
            ClientServices clientService = ServicesProvider.GetInstance().GetClientServices();
            var client = clientService.FindPersonById(message.Id);
            client.IsNew = false;
            client.IsUpdated = false;

            clientService.SavePerson(ref client);

            IClient iClient = ServicesProvider.GetInstance().GetClientServices().FindTiers(client.Id, client.Type);

            if (iClient.Projects != null)
                foreach (Project project in iClient.Projects)
            ...
            if (iClient.Type == OClientTypes.Person)
                InitializePersonForm((Person)iClient, null);
            if (iClient.Type == OClientTypes.Group)
                InitializeGroupForm((Group)iClient, null);
            if (iClient.Type == OClientTypes.Corporate)
                InitializeCorporateForm((Corporate)iClient, null);
            if (OClientTypes.Village == iClient.Type)
                InitializeVillageForm((Village)iClient);           
        }
        ...
        private static bool CanUseTellerManagement()
        {
            var tellers = ServicesProvider.GetInstance().GetTellerServices().FindTellersByUser(User.CurrentUser);
            //User.CurrentUser.AddTellers(tellers);
            return ServicesProvider.GetInstance().GetGeneralSettings().UseTellerManagement 
                /*&& User.CurrentUser.HasCashierRole*/ && tellers.Count > 0;
        }
        ...
        {
            if (CanUseTellerManagement())
            {
                mnuTeller.Visible = true;
        ...
        public void InitializePersonForm(Person person, Project project)
        {
            ClientForm personForm = new ClientForm(person, this, _applicationController)
            {
                ...
                Text = string.Format(
                       ...
                       person.Name,
                       person.Id.ToString().PadLeft(4, '0'))
            };
            if (project != null)
        ...
        public void InitializePersonForm(IClient client)
        {
            switch (client.Type)
            {
                case OClientTypes.Person:
                    {
                        var personForm = new ClientForm((Person)client, this, _applicationController)
                        {
                            ...
                            Text = string.Format(
                               ...
                               MultiLanguageStrings.GetString(Ressource.ClientForm, "Person.Text"),
                               ((Person)client).Name,
                               client.Id.ToString().PadLeft(4, '0'))                            
                        };
                        //personForm.DisplayClient(client, null);
                        personForm.Show();
                        break;
                    }
                case OClientTypes.Group:
                    {
                        ...
                        {
                            MdiParent = this,
                            Text = string.Format("{0} [{1}]", MultiLanguageStrings.GetString(Ressource.ClientForm,
                            "Group.Text"), ((Group)client).Name)
                        };
                        personForm.DisplayClient(client, null);
                        personForm.Show();
                        break;
                ...
                case OClientTypes.Corporate:
                    {
                        ...
                        corporateForm.DisplayClient(client, null);
                        corporateForm.Show();
                        break;
                    }
            }
        ...
        public void InitializeGroupForm(Group group, Project project)
        {
            ClientForm personForm = new ClientForm(group, this, _applicationController)
            {
                ...
                Text =
                    string.Format("{0} [{1} - {2}]", 
                                  MultiLanguageStrings.GetString(Ressource.ClientForm, "Group.Text"),
                                  group.Name,
                                  group.Id.ToString().PadLeft(4, '0'))
            };
            if (project != null)
                personForm.DisplayUserControl_ViewProject(project, null);
            personForm.Show();
                ...
                case OClientTypes.Person:
                    {
                        ...
                        {
                            MdiParent = this,
                            Text = string.Format("{0} [{1} - {2}]", MultiLanguageStrings.GetString(
                            Ressource.ClientForm, "Person.Text"),
                            ((Person)client).Name, ((Person)client).IdentificationData)
                        };
        ...
        public void SetInfoMessage(string pMessage)
        {
            ...
            MessageBox.Show(pMessage, "OpenCBS - Information", MessageBoxButtons.OK, MessageBoxIcon.Information);
        }
        ...
        private static int vaultInOutNotificationTimer = 0;
        private void timer_Tick(object sender, EventArgs e)
        {
            ...
            mainStatusBarLabelAutoLogoutTime.ForeColor = TimeProvider.IsUsingSystemTime ? Color.Black : Color.Red;
            var appTimeOut = ServicesProvider.GetInstance().GetGeneralSettings().ApplicationTimeout;
            mainStatusBarLabelAutoLogoutTime.Text =
                string.Format("Auto logout in: {0}", TimeSpan.FromMilliseconds(logTimer.TimeLeft).ToString("h'h 'm'm 's's'"));

            if (CanUseTellerManagement() && Teller.CurrentTeller != null)
            {
                
                OCurrency currentCashPosition =
                    ServicesProvider.GetInstance().GetTellerServices().GetTellerBalance(Teller.CurrentTeller);
                
                mainStatusBarLblCashPosition.Visible = true;
                if (currentCashPosition < Teller.CurrentTeller.MinAmountTeller || currentCashPosition > Teller.CurrentTeller.MaxAmountTeller)
                {
                    mainStatusBarLblCashPosition.ForeColor = Color.Red;
                    vaultInOutNotificationTimer++;

                    if (vaultInOutNotificationTimer >= 5 * 100)
                    {
                        vaultInOutNotificationTimer = 0;
                        if (currentCashPosition < Teller.CurrentTeller.MinAmountTeller)
                            Notify($"Current cash position below teller minimum. \n\nA vault out operation is required.");
                        if (currentCashPosition > Teller.CurrentTeller.MaxAmountTeller)
                            Notify($"Current cash position exceeds teller maximum. \n\nA vault in operation is required.");
                    }
                }
                else
                    mainStatusBarLblCashPosition.ForeColor = Color.Green;

                mainStatusBarLblCashPosition.Text = $"Current Cash Position: {currentCashPosition.GetFormatedValue(true)}";
            }
        }
        ...
        {
            InitializeCoreMenu();
            ExtensionMenuItems = ExtensionMenuItems.OrderBy(o => o.Index).ToList();
            foreach (var extensionItem in ExtensionMenuItems)
            {
                ToolStripItem parentMenu = mainMenu.Items.Find(extensionItem.Parent, true).FirstOrDefault();
                
                if (parentMenu == null)
                    continue;

                ToolStripMenuItem owner = (ToolStripMenuItem)parentMenu;//.OwnerItem;

                var temp = extensionItem;

                var items = owner == null ? mainMenu.Items : owner.DropDownItems;

                ToolStripItem beforeMenu = mainMenu.Items.Find(extensionItem.InsertAfter, true).FirstOrDefault();
                var index = items.IndexOf(beforeMenu);
                //var index = extensionItem.Index;
            ...
            var appMenu = _applicationController.GetAllInstances<IMenu>();
            foreach (var menu in appMenu)
            {
        ...
        private void InitializeCoreMenu()
        {
            ...
            mainMenu.Items["mnuPayroll"].Visible = ServicesProvider.GetInstance().GetGeneralSettings().EnablePayroll;
        }
        ...
        private void changePasswordToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            User.CurrentUser.Secret.Question = pswdForm.SecretQuestion;
            User.CurrentUser.Secret.Answer = pswdForm.SecretQuestionAnswer;
            try { 
                UserServices.UserErrors userErrors = ServicesProvider.GetInstance()
                    .GetUserServices().SaveUser(User.CurrentUser, true);
        ...
        {
            OpenUrl("http://rskinitiatives.com/en/Additional-Modules/");
        }
        ...
        private void contactMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                const string url = "mailto:contact@zeeandweeta.com?subject=I have a question about OpenCBS";
                Process.Start(url);
            }
            catch
            {
        ...
        private void OpenUserGuid(object sender, EventArgs e)
        {
            try
            {
                const string url = "http://zeeandweeta.com/uploads/User%20guide%20OpenCBS%2014.11.pdf";
                Process.Start(url);
            }
            catch
            {}
        ...
        private void getHelpFromForumToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                const string url = "http://zeeandweeta.freeforums.net/";
                Process.Start(url);
            }
            catch
            { }
        ...
        private void visitOpenCBScomToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                const string url = "http://zeeandweeta.com";
                Process.Start(url);
            }
            catch
            { }
        ...
        {
            SavingsCodeForm frm = new SavingsCodeForm();
            if (DialogResult.OK == frm.ShowDialog())
            {
                ServicesProvider.GetInstance().GetGeneralSettings().UpdateParameter(OGeneralSettings.SAVINGS_CODE_TEMPLATE, frm.code);
                ServicesProvider.GetInstance().GetApplicationSettingsServices().UpdateSelectedParameter(OGeneralSettings.SAVINGS_CODE_TEMPLATE, frm.code);
            }
        }

        private void miMessaging_Click(object sender, EventArgs e)
        {
            MessagingForm frm = new MessagingForm { MdiParent = this };
            frm.Show();
        }

        private static TimerPlus logTimer;
        
        private void LogOutTimer_Elapsed(object sender, System.Timers.ElapsedEventArgs e)
        {
            User.CurrentUser.TimedOut = true;
            User.CurrentUser.IsExpired = false;

            try
            {
                logTimer.Stop();
                ServicesProvider.GetInstance().GetUserServices()
                    .UpdateUserAccess(User.CurrentUser);
                Restart = true;
                Close();
            }
            catch (Exception ex)
        ...
        private void miTellerHistory_Click(object sender, EventArgs e)
        {
            FrmTellerHistory tellerHistoryForm = new FrmTellerHistory { MdiParent = this };
            tellerHistoryForm.Show();
        }

        private void miVaultTransfer_Click(object sender, EventArgs e)
        {
            FrmTellerOperation tellerOperationForm = new FrmTellerOperation();
            tellerOperationForm.ShowDialog();
        }
    }
}
