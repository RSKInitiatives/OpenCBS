...
namespace OpenCBS.Services
{
    ...
    /// </summary>
    [Security()]
    public class AccountServices : Services
    {
        ...
        public AccountServices(User pUser)
        {
            _user = pUser;
        ...
        public AccountServices(string pTestDB)
        {
        ...
        public AccountServices(string pTestDB, User pUser)
        {
            _user = pUser;
            _savingManager = new SavingManager(pTestDB);
            _savingEventManager = new SavingEventManager(pTestDB);
        ...
        public AccountServices(SavingManager pSavingManager, SavingEventManager pSavingEventManager, User pUser)
        {
            _user = pUser;
        ...
        public AccountServices(SavingManager pSavingManager, SavingEventManager pSavingEventManager, LoanManager pLoanManager, User pUser)
        {
            _user = pUser;
            _savingManager = pSavingManager;
            _savingEventManager = pSavingEventManager;
        ...
        private static bool IsInstrumentFeesCorrect(SavingBookContract pSaving)
        {
            if (!pSaving.InstrumentFees.HasValue)
                return false;

            if (pSaving.Product.InstrumentFees.HasValue)
                return pSaving.InstrumentFees.Value == pSaving.Product.InstrumentFees;

            return ServicesHelper.CheckIfValueBetweenMinAndMax(pSaving.Product.InstrumentFeesMin,
                                                               pSaving.Product.InstrumentFeesMax, pSaving.InstrumentFees);
        }

        private static bool IsSearchFeesCorrect(SavingBookContract pSaving)
        {
            if (!pSaving.SearchFees.HasValue)
                return false;

            if (pSaving.Product.SearchFees.HasValue)
                return pSaving.SearchFees.Value == pSaving.Product.SearchFees;

            return ServicesHelper.CheckIfValueBetweenMinAndMax(pSaving.Product.SearchFeesMin,
                                                               pSaving.Product.SearchFeesMax, pSaving.SearchFees);
        }

        private static bool IsOtherFeesCorrect(SavingBookContract pSaving)
        {
            if (!pSaving.DutyStampFees.HasValue)
                return false;

            if (pSaving.Product.DutyStampFees.HasValue)
                return pSaving.DutyStampFees.Value == pSaving.Product.DutyStampFees;

            return ServicesHelper.CheckIfValueBetweenMinAndMax(pSaving.Product.DutyStampFeesMin,
                                                               pSaving.Product.DutyStampFeesMax, pSaving.DutyStampFees);
        }
        ...
        {
            // Virtual balance
            saving.Loans = ServicesProvider.GetInstance().GetAccountServices().SelectLoansBySavingsId(saving.Id);
        ...
        public List<SavingEvent> Deposit(ISavingsContract saving, DateTime dateTime, OCurrency depositAmount,
            string description, string referenceNumber, User user, bool isPending, bool? isLocal, OSavingsMethods savingsMethod, PaymentMethod paymentMethod, int? pendingEventId, Teller teller)
        {
            using (SqlConnection conn = _savingManager.GetConnection())
            using (SqlTransaction sqlTransaction = conn.BeginTransaction())
            {
                try
                {
                    ...
                    savingSimulation.Deposit(depositAmount, dateTime, description, referenceNumber, user, false, isPending, isLocal, savingsMethod,
                    ...
                    List<SavingEvent> events = saving.Deposit(depositAmount, dateTime, description, referenceNumber, user, false,
                                                              isPending, isLocal,
        ...
        public List<SavingEvent> Deposit(SavingBookContract _saving, DateTime _date, decimal amountToPay, object description, object referenceNumber, User currentUser, bool pending, bool? local, OSavingsMethods savingsMethod, PaymentMethod paymentMethod, object p, Teller currentTeller)
        {
        ...
        /// <returns></returns>
        public List<SavingEvent> Withdraw(ISavingsContract pSaving, DateTime pDate, OCurrency pWithdrawAmount, string pDescription, string pReferenceNumber, User pUser, Teller teller, PaymentMethod paymentMethod)
        {
            ValidateWithdrawal(pWithdrawAmount, pSaving, pDate, pDescription, pReferenceNumber, pUser, teller, paymentMethod);
            if (pSaving.Client == null)
                pSaving.Client =
                    ServicesProvider.GetInstance().GetClientServices().FindTiersBySavingsId(pSaving.Id);
            List<SavingEvent> events = pSaving.Withdraw(pWithdrawAmount, pDate, pDescription, pReferenceNumber, pUser, false, teller, paymentMethod);
            using (SqlConnection conn = _savingManager.GetConnection())
            using (SqlTransaction sqlTransaction = conn.BeginTransaction())
            {
                try
                {
                        ...
                        ServicesProvider.GetInstance()
                            ...
                            .CallInterceptor(new Dictionary<string, object>
                            {
                                ...
                                {"Event", new SavingWithdrawEvent
                                {
                                    ...
                                    ReferenceNumber = savingEvent.ReferenceNumber,
                    ...
                    if (pSaving is SavingBookContract)
                    {
                        ...
                        {
                            SavingEvent overdraftFeeEvent = pSaving.ChargeOverdraftFee(pDate, pUser, pReferenceNumber);
        ...
        public List<SavingEvent> Withdraw(ISavingsContract pSaving, DateTime pDate, OCurrency pWithdrawAmount, bool isDeactivateFees, string pDescription, string pReferenceNumber, User pUser, Teller teller, PaymentMethod paymentMethod)
        {
            ValidateWithdrawal(pWithdrawAmount, pSaving, pDate, pDescription, pReferenceNumber, pUser, teller, paymentMethod);
            if (pSaving.Client == null)
                pSaving.Client =
                    ServicesProvider.GetInstance().GetClientServices().FindTiersBySavingsId(pSaving.Id);
            List<SavingEvent> events = pSaving.Withdraw(pWithdrawAmount, pDate, pDescription, pReferenceNumber, pUser, isDeactivateFees, teller, paymentMethod);
            using (SqlConnection conn = _savingManager.GetConnection())
            using (SqlTransaction sqlTransaction = conn.BeginTransaction())
            {
                try
        ...
        public List<SavingEvent> Withdraw(ISavingsContract pSaving, DateTime pDate, OCurrency pWithdrawAmount,
                                          string pDescription, string pReferenceNumber, User pUser, Teller teller, PaymentMethod paymentMethod, SqlTransaction sqlTransaction)
        {
            ValidateWithdrawal(pWithdrawAmount, pSaving, pDate, pDescription, pReferenceNumber, pUser, teller, paymentMethod);
        ...
        private void ValidateWithdrawal(OCurrency pWithdrawAmount, ISavingsContract pSaving, DateTime pDate, string pDescription, string pReferenceNumber, User pUser, Teller teller, PaymentMethod paymentMethod)
        {
            ...
            savingSimulation.Withdraw(pWithdrawAmount, pDate, pDescription, pReferenceNumber, pUser, false, teller, paymentMethod);
            // Check balance simulation
            if (!IsSavingBalanceCorrect(savingSimulation))
                throw new OpenCbsSavingException(OpenCbsSavingExceptionEnum.BalanceIsInvalid);
        }

        public List<SavingEvent> Transfer(ISavingsContract from, ISavingsContract to, DateTime date, OCurrency amount, OCurrency fee, string description, string referenceNumber, User user, bool noFee)
        {
            CheckTransfer(to, from, amount, fee, date, description, referenceNumber);
            List<SavingEvent> events = from.Transfer(to, amount, fee, date, description, referenceNumber);
        ...
        private void CheckTransfer(ISavingsContract to, ISavingsContract from, OCurrency amount, OCurrency fee, DateTime date, string description, string referenceNumber)
        {
            if (to == null)
            ...
            fromCopy.Transfer(toCopy, amount, fee, date, description, referenceNumber);
        ...
        public List<SavingEvent> CancelLastEvent(ISavingsContract saving, User user, string pDescription)
        {
            List<SavingEvent> canceledEvents = new List<SavingEvent>();
            ...
            lastSavingEvent.IsPending = false;
            ...
            if (lastSavingEvent.PendingEventId != null)
            {
                ...
                canceledEvents.Add(savingEvent);                
            }

            //slycode: Cancel last event fee event collection
            List<SavingEvent> feeEvents = saving.GetCancelableFeeEvent(lastSavingEvent);
            foreach (SavingEvent feeEvent in feeEvents)
            {
                feeEvent.Description = pDescription;
                feeEvent.IsPending = false;
                _savingEventManager.UpdateEventDescription(feeEvent.Id, pDescription);
                _ePS.CancelFireEvent(feeEvent, saving.Product.Currency.Id);
                canceledEvents.Add(feeEvent); 
            }
            ...
            bool isDesactivateFees, Teller teller, PaymentMethod paymentMethod)
        {
            ...
            if ((balance.Value - withdrawAmount.Value) > (decimal)0.1)
            {
                throw new OpenCbsSavingException(OpenCbsSavingExceptionEnum.WithdrawAmountIsInvalid);
            }
        ...
        public List<SavingEvent> SelectEventsForClosure(DateTime beginDate, DateTime endDate, Branch branch)
        {
            ...
            */
            //returns 12AM value of next day, above code doesn't work
            //endDate.AddDays(1);
        ...
        public void ChangePendingEventStatus(int savingEventId, bool isPending, SqlTransaction transaction)
        {
            _savingEventManager.ChangePendingEventStatus(savingEventId, isPending, transaction);
        }
        ...
        public ISavingsContract GetSaving(int savingId)
        {
            ISavingsContract saving = _savingManager.Select(savingId);
            if (saving != null)
            {
        ...
        public bool ValidateSavingsContract(ISavingsContract saving, Client client)
        {
            ...
            if (!IsInstrumentFeesCorrect((SavingBookContract)saving))
                throw new OpenCbsSavingException(OpenCbsSavingExceptionEnum.InstrumentFeesIsInvalid);
            if (!IsSearchFeesCorrect((SavingBookContract)saving))
                throw new OpenCbsSavingException(OpenCbsSavingExceptionEnum.SearchFeesIsInvalid);
            if (!IsOtherFeesCorrect((SavingBookContract)saving))
                throw new OpenCbsSavingException(OpenCbsSavingExceptionEnum.OtherFeesIsInvalid);
        ...
        public int SaveContract(ISavingsContract saving, Client client, Action<SqlTransaction, int> action)
        {
            ...
            using (SqlTransaction sqlTransaction = connection.BeginTransaction())
            {
                try
                {
                    ...
                    action?.Invoke(sqlTransaction, saving.Id);
        ...
        public void CanPostAfterClosureEvent()
        {
            return;
        }
        ...
        {
            //if(User.CurrentUser.Role.HasValue && User.CurrentUser.Role.Value != User.Roles.ADMIN)
              //  throw new OpenCbsSavingException(OpenCbsSavingExceptionEnum.SavingBackDateDenied);
            return;
        }
        ...
        {
            //if (User.CurrentUser.Role.HasValue && User.CurrentUser.Role.Value != User.Roles.ADMIN)
              //  throw new OpenCbsSavingException(OpenCbsSavingExceptionEnum.SavingFutureDateDenied);
            return;
        }
