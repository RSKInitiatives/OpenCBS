...
namespace OpenCBS.Services.Accounting
{
    public class AccountingServices : ContextBoundObject
	{
                        ...
                        foreach (Booking booking in bookings)
                        {
                            switch (booking.Type)
                            {
                                ...
                                case OMovementType.Saving:
                                    _movementSetManagement.InsertSavingMovement(booking, sqlTransac);
                                    ServicesProvider.GetInstance().GetAccountServices().MakeEventExported(
                                        booking.EventId,
                                        sqlTransac);

                                    //slycode: Update event pending state after movement and export
                                    ServicesProvider.GetInstance().GetAccountServices().ChangePendingEventStatus(
                                        booking.EventId, false, sqlTransac);
                                    break;
                                case OMovementType.Manual:
                                    _movementSetManagement.UpdateManualMovement(booking, sqlTransac);
                                    break;
        ...
        {
            return _movementSetManagement.GetAllAccountBalances(date, branchId);
        }
