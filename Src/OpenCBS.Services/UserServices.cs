...
namespace OpenCBS.Services
{
    public class UserServices : MarshalByRefObject
    {
        ...
        public struct UserErrors
        {
            ...
            public bool SecretQuestionError;
            public bool SecretQuestionAnswerError;
        }
        ...
        public bool UpdateUserAccess(User pUser) {
            _userManager.UpdateUserAccess(pUser);
            return true;
        }
        ...
        public User FindByUserName(string userName)
        {
            ...
            LoadUsers();
            Debug.Assert(_users != null, "User list is null");
            return _users.Find(item => item.UserName == userName);
        }

        public User FindByFrapidId(int id)
        {
            LoadUsers();
            Debug.Assert(_users != null, "User list is null");
            return _users.Find(item => item.FrapidLoginId.HasValue && item.FrapidLoginId.Value == id);
        }
        ...
        public User Find(string username, string password)
        {
            ...
            return u;
             */
            User u = _users.Find(item => item.UserName == username && !item.IsDeleted);
            bool validate = u != null && Hasher.Validate(password, u.Password);
            return validate ? u : null;            
        }

        public User Find(string username, string password, string sqa)
        {
            LoadUsers();
            Debug.Assert(_users != null, "User list is null");
            /*User u = _users.Find(item => item.UserName == username
            ...
            if (u != null)
            {
                u.Secret.Valid = Hasher.Validate(sqa, u.Secret.Answer);
                validate = (Hasher.Validate(password, u.Password) && u.Secret.Valid) || (u.IsReset.HasValue && u.IsReset.Value);
                if (!validate)
                {
                    u.LoginAttempt++;
                    UpdateUserAccess(u);
