...
namespace OpenCBS.Manager.Accounting
{
    ...
    public class AccountingTransactionManager : Manager
    {
        ...
        public DataSet GetAllAccountBalances(DateTime date, int branchId)
        {
            using (SqlConnection conn = GetConnection())
            {
                using (OpenCbsCommand cmd = new OpenCbsCommand("GetAllAccountBalances", conn).AsStoredProcedure())
                {
                    ...
                    using (OpenCbsReader reader = cmd.ExecuteReader())
                    {
                        if (reader.Empty) return dataset;//accountBalances;
                        OCurrency balance = 0;
                        //while (reader.Read())
                        //{
                        ...
                        //    //    type_code = reader.GetString("type_code"),
                        //    //    balance = reader.GetMoney("balance")
                        //    //});
                        //}
                        dataset.Tables.Add("AllAccountBalances");
                        dataset.Tables[0].Load(reader.Reader);

                        return dataset;// accountBalances;
                    }
                }
            }
        }
        ...
        public void InsertSavingMovement(Booking booking, SqlTransaction sqlTransaction)
        {
            const string sqlText =
        ...
        public void UpdateManualMovement(Booking booking, SqlTransaction sqlTransaction)
        {
            const string sqlText =
