...
namespace OpenCBS.Manager
{
    public class UserManager : Manager
    {
        ...
        public void UpdateUser(User pUser, bool saveSecret = false)
        {
            string sqlText = @"UPDATE [Users] 
            ...
            using (OpenCbsCommand sqlCommand = new OpenCbsCommand(sqlText, conn))
            {
                ...
                SetUser(sqlCommand, pUser, saveSecret);
                sqlCommand.ExecuteNonQuery();
                _UpdateUsersRole(pUser.Id, pUser.UserRole.Id);
            }
        }

        private static void SetUser(OpenCbsCommand sqlCommand, User pUser, bool saveSecret = false)
        {
            sqlCommand.AddParam("@now", DateTime.Now);
            sqlCommand.AddParam("@username", pUser.UserName);
            if (saveSecret)
            {
                sqlCommand.AddParam("@userpass", Hasher.Get(pUser.Password));
                sqlCommand.AddParam("@usersq", pUser.Secret.Question);
                sqlCommand.AddParam("@usersqa", Hasher.Get(pUser.Secret.Answer));
            }
            sqlCommand.AddParam("@roleCode", pUser.UserRole.ToString());
        ...
        public void UpdateUserAccess(User pUser)
        {
            ...
            using (OpenCbsCommand sqlCommand = new OpenCbsCommand(sqlText, conn))
            {
                ...
                sqlCommand.AddParam("@timedout", pUser.TimedOut);
                sqlCommand.AddParam("@isexpired", pUser.IsExpired);
                sqlCommand.AddParam("@login_attempt", pUser.LoginAttempt);
                sqlCommand.AddParam("@isreset", pUser.IsReset);
                sqlCommand.ExecuteNonQuery();
                _UpdateUsersRole(pUser.Id, pUser.UserRole.Id);
            }
