...
namespace OpenCBS.Manager
{
    public class PicturesManager : Manager
    {
            ...
            string path)
        {
            ...
            // Add row
            string q =
                @"INSERT INTO Pictures ([group], [id] ,[subid] ,[picture] ,[thumbnail] ,[name], [path]) 
                VALUES (@group ,@id ,@subid ,@picture ,@thumbnail ,@name, @path)";
            using (OpenCbsCommand c = new OpenCbsCommand(q, AttachmentsConnection))
            {
                c.AddParam("group", pGroup);
                c.AddParam("id", pId);
        ...
        public void UpdatePicture(string group, int personId, int pictureSubId, string fileName, byte[] picture,
            byte[] thumbnail, string path)
        {
            string q = string.Format(@"
                    UPDATE Pictures 
                    SET picture=@picture, thumbnail=@thumbnail, name=@file_name, path=@path
