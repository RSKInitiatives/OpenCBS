    ...
    public class TellerManager : Manager
    {
        ...
        public Teller Add(Teller teller, SqlTransaction t)
        {
            const string sqlText =
                @"INSERT INTO dbo.Tellers (name, description, account_id, branch_id, user_id, currency_id,
                                            amount_min, amount_max, deposit_amount_min, deposit_amount_max, withdrawal_amount_min, withdrawal_amount_max) 
                                   VALUES (@name, @description, @account_id, @branch_id, @user_id, @currency_id,
                                            @amount_min, @amount_max, @deposit_amount_min, @deposit_amount_max, @withdrawal_amount_min, @withdrawal_amount_max) 
            ...
            using (var c = new OpenCbsCommand(sqlText, t.Connection, t))
            {
                ...
                c.AddParam("@description", teller.Description);
        ...
        public OCurrency GetCurrentCashPosition(Teller teller)
        {
            using (SqlConnection conn = GetConnection())
            {
                using (var c = new OpenCbsCommand("GetCashPosition", conn).AsStoredProcedure())
                {
                    c.AddParam("@teller_id", teller.Id);
