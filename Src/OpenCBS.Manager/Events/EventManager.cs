...
namespace OpenCBS.Manager.Events
{
	public class EventManager : Manager
	{
        ...
        {
            const string q = @"SELECT * FROM dbo.TellerEventsHistory(@from, @to, @user_id, @teller_id, @branch_id, @del)";

            List<TellerSavingEvent> retval = new List<TellerSavingEvent>();
            using (SqlConnection conn = GetConnection())
            using (OpenCbsCommand c = new OpenCbsCommand(q, conn))
            {
                c.AddParam("@from", filter.From);
                ...
                c.AddParam("@user_id", filter.UserId);
                c.AddParam("@branch_id", filter.BranchId);
                c.AddParam("@teller_id", filter.TellerId);
                c.AddParam("@del", filter.IncludeDeleted);
                using (OpenCbsReader r = c.ExecuteReader())
                {
                    if (r.Empty) return retval;
                    ...
                    while (r.Read())
                    {                        
                        retval.Add(GetTellerSavingEvent(r));
                    }
                }
            }
        ...
        private static TellerSavingEvent GetTellerSavingEvent(OpenCbsReader r)
        {
            TellerSavingEvent e;
            ...
            switch (r.GetString("code"))
            {
                case OSavingEvents.Deposit:
                    e = new TellerSavingPositiveEvent();
                    break;
                case OSavingEvents.Withdraw:
                    e = new TellerSavingNegativeEvent();
                    break;
            ...
            e.Date = r.GetDateTime("date");
            e.Fee = r.GetDecimal("fees");
            e.Amount = r.GetDecimal("amount");
            e.Code = r.GetString("code");
            e.Account = r.GetString("account");
            ...
            e.UserName = r.GetString("user_name");
            e.TellerName = r.GetString("teller_name");
            e.BranchName = r.GetString("branch_name");            
            return e;
        }
