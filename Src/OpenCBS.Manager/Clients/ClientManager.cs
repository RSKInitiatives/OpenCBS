    ...
    public class ClientManager : Manager
    {
        ...
        public int AddPerson(Person pPerson, SqlTransaction pSqlTransac)
        {
            ...
            const string q = @"INSERT INTO [Persons]
                            ...
                            [particulars_type],
                            [particulars_id_number],
                            [particulars_issue_date],
                            [particulars_expiry_date],
            ...
            using (OpenCbsCommand c = new OpenCbsCommand(q, pSqlTransac.Connection, pSqlTransac))
            {
                c.AddParam("@isNew", pPerson.IsNew);
                c.AddParam("@isUpdated", pPerson.IsUpdated);
                c.AddParam("@smsDelivery", pPerson.SmsDelivery);
                c.AddParam("@emailDelivery", pPerson.EmailDelivery);
                c.AddParam("@BirthPlace", pPerson.BirthPlace);
                c.AddParam("@Nationality", pPerson.Nationality);

                c.AddParam("@particulars_type", pPerson.ParticularsType);
                c.AddParam("@particulars_id_number", pPerson.ParticularsIDNumber);
                c.AddParam("@particulars_issue_date", pPerson.ParticularsIssueDate);
                c.AddParam("@particulars_expiry_date", pPerson.ParticularsExpiryDate);
                ...
                if (pPerson.RelationshipOfficer != null)
                    c.AddParam("@relationship_officer_id", pPerson.RelationshipOfficer.Id);
                else
                    c.AddParam("@relationship_officer_id", DBNull.Value);
        ...
        public Person SelectPersonById(int pPersonId, bool loadLoans)
        {
            ...
            string q = @"SELECT 
                                ...
                                (SELECT TOP(1) Pictures.path
                                                    FROM Pictures
                                                    WHERE Pictures.[group]='PERSON' 
                                                    AND Pictures.[id]=Persons.id 
                                                    AND Pictures.[subid]=0) as photo1,
	                            (SELECT TOP(1) Pictures.path
                                                    FROM Pictures
                                                    WHERE Pictures.[group]='PERSON' 
                                                    AND Pictures.[id]=Persons.id 
                                                    AND Pictures.[subid]=1) as photo2,
	                            (SELECT TOP(1) Pictures.path
                                                    FROM Pictures
                                                    WHERE Pictures.[group]='PERSON' 
                                                    AND Pictures.[id]=Persons.id 
                                                    AND Pictures.[subid]=2) as [signature],

                                 Persons.father_name, 
                                 Persons.birth_place, 
                                 Persons.nationality,
                                 Persons.relationship_officer_id,
                                 Persons.loan_officer_id,
                                 Persons.particulars_type,
                                 Persons.particulars_id_number,
                                 Persons.particulars_issue_date,
                                 Persons.particulars_expiry_date,
                                 Persons.sms_delivery,
                                 Persons.email_delivery,
                                 Persons.is_new,
                                 Persons.is_updated     
                            FROM Tiers 
            ...
            if (person != null)
            {
                ...
                if (person.FavouriteLoanOfficerId.HasValue)
                    person.FavouriteLoanOfficer = userManager.SelectUser((int)person.FavouriteLoanOfficerId,
                                                                         true);
                if (person.RelationshipOfficerId.HasValue)
                    person.RelationshipOfficer = userManager.SelectUser((int)person.RelationshipOfficerId,
                                                                         true);
            ...
            person = new Person
                         {
                             ...
                             ImagePath = r.GetString("photo1"),
                             Image2Path = r.GetString("photo2"),
                             Image3Path = r.GetString("signature"),
                             BirthPlace = r.GetString("birth_place"),
                             Nationality = r.GetString("nationality"),
                             ParticularsType = (OParticularsType)r.GetInt("particulars_type"),
                             ParticularsIDNumber = r.GetString("particulars_id_number"),
                             ParticularsIssueDate = r.GetNullDateTime("particulars_issue_date"),
                             ParticularsExpiryDate = r.GetNullDateTime("particulars_expiry_date"),
                             ...
                             RelationshipOfficerId = r.GetNullInt("relationship_officer_id"),
                             Branch = new Branch { Id = r.GetInt("branch_id") },
                         };

            try
            {
                person.IsNew = r.GetNullBool("is_new");
                person.IsUpdated = r.GetNullBool("is_updated");

                person.SmsDelivery = r.GetNullBool("sms_delivery");
                person.EmailDelivery = r.GetNullBool("email_delivery");
            }
            catch { };
            return person;
        }
        ...
        public void UpdatePerson(Person person, SqlTransaction sqlTransac)
        {
            ...
            const string q = @"UPDATE [Persons] SET 
                                ...
                                ,[particulars_type] = @particulars_type
                                ,[particulars_id_number] = @particulars_id_number
                                ,[particulars_issue_date] = @particulars_issue_date
                                ,[particulars_expiry_date] = @particulars_expiry_date
                                ,[is_new] = @isNew
                                ,[is_updated] = @isUpdated
                                ,[sms_delivery] = @smsDelivery
                                ,[email_delivery] = @emailDelivery
                                ,[relationship_officer_id] = @relationship_officer_id
                                WHERE id = @id";
            ...
            using (OpenCbsCommand c = new OpenCbsCommand(q, sqlTransac.Connection, sqlTransac))
            {
                ...
                c.AddParam("@isNew", person.IsNew);
                c.AddParam("@isUpdated", person.IsUpdated);
                c.AddParam("@smsDelivery", person.SmsDelivery);
                c.AddParam("@emailDelivery", person.EmailDelivery);
                ...
                c.AddParam("@particulars_type", person.ParticularsType);
                c.AddParam("@particulars_id_number", person.ParticularsIDNumber);
                c.AddParam("@particulars_issue_date", person.ParticularsIssueDate);
                c.AddParam("@particulars_expiry_date", person.ParticularsExpiryDate);

                c.AddParam("@image", person.Image);
                if (person.RelationshipOfficer != null)
                    c.AddParam("@relationship_officer_id", person.RelationshipOfficer.Id);
                else
                    c.AddParam("@relationship_officer_id", DBNull.Value);
        ...
        public List<ClientSearchResult> SearchCorporateByCriteres(int onlyActive, int pageNumber, string pQuery)
        {
            ...
            string CloseWhere = @" WHERE ( siret LIKE @siret OR id LIKE @id OR name LIKE @name OR district LIKE @district OR city LIKE @city )) maTable";
                            ...
                            var result = new ClientSearchResult
                                             {
                                                 ...
                                                 PassportNumber = Convert.ToString(r.GetInt("id")),
            ...
            using (OpenCbsReader r = c.ExecuteReader())
            {
                if (!r.Empty)
                {
                    while (r.Read())
                    {
                        ...
                        {
                            result.PassportNumber = Convert.ToString(r.GetInt("passport_number"));
                        }
            ...
            sqlText += string.Format(@"
		         ( 
			         ...
			         SELECT  Villages.id
                            ...
                            ,Tiers.bad_client AS bad_client
                            /*,'-' AS passport_number*/
                            ,'-' + Villages.id AS passport_number                            
                            ,'-' AS village_name 
			         FROM Villages 
				          INNER JOIN Tiers ON Villages.id = Tiers.id
				          INNER JOIN Districts ON Districts.id = Tiers.district_id
			         WHERE (Villages.id LIKE @passport OR Villages.name LIKE @name OR Districts.name  LIKE  @district OR Tiers.city  LIKE @city ) 
			         ...
			         SELECT Groups.id
                           ...
                           ,Tiers.bad_client AS bad_client
                           /*,'-' AS passport_number*/
                           ,'-' + Groups.id AS passport_number                            
                           ,'-' AS group_name 
			         FROM Groups
				          INNER JOIN Tiers ON Groups.id = Tiers.id 
				          INNER JOIN Districts ON Districts.id = Tiers.district_id 
			         WHERE (Groups.id LIKE @passport OR Groups.name LIKE @name OR Districts.name LIKE  @district OR Tiers.city LIKE @city ) 
