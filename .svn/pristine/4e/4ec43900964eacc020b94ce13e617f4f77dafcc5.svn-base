    ...
    public class SavingProductManager : Manager
    {
        ...
        private void UpdateBookProduct(SavingsBookProduct product, bool inUse)
        {
            string q = @"UPDATE [SavingBookProducts] SET 
                                    ...
                                    ,[flat_am_fees_min] = @flatAMFeesMin 
                                    ,[flat_am_fees_max] = @flatAMFeesMax 
                                    ,[flat_am_fees] = @flatAMFees 
                                    ,[rate_am_fees_min] = @rateAMFeesMin 
                                    ,[rate_am_fees_max] = @rateAMFeesMax 
                                    ,[rate_am_fees] = @rateAMFees
                                    ...
                                    ,[instrument_fees_max] = @instrumentFeesMax
                                    ,[instrument_fees_min] = @instrumentFeesMin
                                    ,[instrument_fees] = @instrumentFees
                                    ,[search_fees_max] = @searchFeesMax
                                    ,[search_fees_min] = @searchFeesMin
                                    ,[search_fees] = @searchFees
                                    ,[duty_stamp_fees_max] = @dutyStampFeesMax
                                    ,[duty_stamp_fees_min] = @dutyStampFeesMin
                                    ,[duty_stamp_fees] = @dutyStampFees
       ...
       public void UpdateExistingSavingBooksContracts(ISavingProduct product)
        {
            ...
            if (savingsBookProduct.InstrumentFees.HasValue)
                updateExistingContractsSql += ",[flat_instrument_fees]=@instrument_fees";

            if (savingsBookProduct.SearchFees.HasValue)
                updateExistingContractsSql += ",[flat_search_fees]=@search_fees";

            if (savingsBookProduct.DutyStampFees.HasValue)
                updateExistingContractsSql += ",[flat_duty_stamp_fees]=@duty_stamp_fees";

            if (savingsBookProduct.FlatAMFees.HasValue)
                updateExistingContractsSql += ",[flat_am_fees]=@flat_am_fees";

            if (savingsBookProduct.RateAMFees.HasValue)
                updateExistingContractsSql += ",[flat_am_fees]=@rate_am_fees";
            ...
            if (savingsBookProduct.RateWithdrawFees.HasValue)
                updateExistingContractsSql += ",[rate_withdraw_fees]=@rate_withdraw_fees";            
            ...
            foreach (int contractId in contractsId)
            {
                ...
                using (OpenCbsCommand c = new OpenCbsCommand(updateExistingContractsSql, conn))
                {
                    ...
                    if (savingsBookProduct.InstrumentFees.HasValue)
                        c.AddParam("@instrument_fees", savingsBookProduct.InstrumentFees.Value);
                    if (savingsBookProduct.SearchFees.HasValue)
                        c.AddParam("@search_fees", savingsBookProduct.SearchFees.Value);
                    if (savingsBookProduct.DutyStampFees.HasValue)
                        c.AddParam("@duty_stamp_fees", savingsBookProduct.DutyStampFees.Value);
                    if (savingsBookProduct.FlatAMFees.HasValue)
                        c.AddParam("@flat_am_fees", savingsBookProduct.FlatAMFees.Value);
                    if (savingsBookProduct.RateAMFees.HasValue)
                        c.AddParam("@rate_am_fees", savingsBookProduct.RateAMFees.Value);
                    if (savingsBookProduct.DepositFees.HasValue)
                    ...
                    if (savingsBookProduct.RateWithdrawFees.HasValue)
                        c.AddParam("@rate_withdraw_fees", savingsBookProduct.RateWithdrawFees.Value);                    
                    if (savingsBookProduct.FlatTransferFees.HasValue)
        ...
        private static ISavingProduct GetProduct(OpenCbsReader r)
        {
            ...
            product.AMMin = r.GetMoney("am_min");
            product.AMMax = r.GetMoney("am_max");
            ...
            if (product is SavingsBookProduct)
            {
                ...
                savingBookProduct.AMFeesType = (OSavingsFeesType)r.GetSmallInt("am_fees_type");
                if (savingBookProduct.AMFeesType == OSavingsFeesType.Flat)
                {
                    savingBookProduct.FlatAMFeesMin = r.GetMoney("flat_am_fees_min");
                    savingBookProduct.FlatAMFeesMax = r.GetMoney("flat_am_fees_max");
                    savingBookProduct.FlatAMFees = r.GetMoney("flat_am_fees");
                }
                else
                {
                    savingBookProduct.RateAMFeesMin = r.GetNullDouble("rate_am_fees_min");
                    savingBookProduct.RateAMFeesMax = r.GetNullDouble("rate_am_fees_max");
                    savingBookProduct.RateAMFees = r.GetNullDouble("rate_am_fees");
                }
                ...
                savingBookProduct.InstrumentFees = r.GetMoney("instrument_fees");
                savingBookProduct.InstrumentFeesMax = r.GetMoney("instrument_fees_max");
                savingBookProduct.InstrumentFeesMin = r.GetMoney("instrument_fees_min");
                savingBookProduct.SearchFees = r.GetMoney("search_fees");
                savingBookProduct.SearchFeesMax = r.GetMoney("search_fees_max");
                savingBookProduct.SearchFeesMin = r.GetMoney("search_fees_min");
                savingBookProduct.DutyStampFees = r.GetMoney("duty_stamp_fees");
                savingBookProduct.DutyStampFeesMax = r.GetMoney("duty_stamp_fees_max");
                savingBookProduct.DutyStampFeesMin = r.GetMoney("duty_stamp_fees_min");
        ...
        private static void SetProduct(OpenCbsCommand c, ISavingProduct product)
        {
            ...
            if (product is  SavingsBookProduct)
            {
                ...
                c.AddParam("@instrumentFees", product.InstrumentFees);
                c.AddParam("@instrumentFeesMax", product.InstrumentFeesMax);
                c.AddParam("@instrumentFeesMin", product.InstrumentFeesMin);

                c.AddParam("@searchFees", product.SearchFees);
                c.AddParam("@searchFeesMax", product.SearchFeesMax);
                c.AddParam("@searchFeesMin", product.SearchFeesMin);

                c.AddParam("@dutyStampFees", product.DutyStampFees);
                c.AddParam("@dutyStampFeesMax", product.DutyStampFeesMax);
                c.AddParam("@dutyStampFeesMin", product.DutyStampFeesMin);

                c.AddParam("@amMin", product.AMMin);
                c.AddParam("@amMax", product.AMMax);
        ...
        private static void SetProduct(OpenCbsCommand c, SavingsBookProduct product)
        {
            ...
            c.AddParam("@amFeesType", (int)product.AMFeesType);
            c.AddParam("@flatAMFeesMin", product.AMFeesType == OSavingsFeesType.Flat ? product.FlatAMFeesMin : null);
            c.AddParam("@flatAMFeesMax", product.AMFeesType == OSavingsFeesType.Flat ? product.FlatAMFeesMax : null);
            c.AddParam("@flatAMFees", product.AMFeesType == OSavingsFeesType.Flat ? product.FlatAMFees : null);
            c.AddParam("@rateAMFeesMin", product.AMFeesType == OSavingsFeesType.Rate ? product.RateAMFeesMin : null);
            c.AddParam("@rateAMFeesMax", product.AMFeesType == OSavingsFeesType.Rate ? product.RateAMFeesMax : null);
            c.AddParam("@rateAMFees", product.AMFeesType == OSavingsFeesType.Rate ? product.RateAMFees : null);
