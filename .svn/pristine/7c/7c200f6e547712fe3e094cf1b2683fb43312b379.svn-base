...
namespace OpenCBS.CoreDomain.Contracts.Savings
{
	...
	public class SavingBookContract : SavingsContract
    {
        ...
        public OCurrency FlatAMFees { get; set; }
        public double? RateAMFees { get; set; }
        ...
        public OCurrency InstrumentFees { get; set; }
        public OCurrency SearchFees { get; set; }
        public OCurrency DutyStampFees { get; set; }
        ...
        public override List<SavingEvent> Withdraw(OCurrency pAmount, DateTime pDate, string pDescription, string preferenceNumber, User pUser, bool pIsDesactivateFees, Teller teller, PaymentMethod paymentMethod)
        {
            List<SavingEvent> events = new List<SavingEvent>();
            ...
            if (Status != OSavingsStatus.Closed && !pIsDesactivateFees)
            {
                List<SavingEvent> amFeeEvents = ChargeAMFee(withdrawEvent);
                events.AddRange(amFeeEvents);
        ...
        public override SavingOverdraftFeeEvent ChargeOverdraftFee(DateTime pDate, User pUser, string pReferenceNumber)
        {
        ...
        private List<SavingEvent> GenerateInstrumentFeeEvent(SavingEvent savingsEvent)
        {
            List<SavingEvent> retval = new List<SavingEvent>();

            const string desc = "Instrument fee of {0:.00} : {1}";
            object[] items = new object[] { InstrumentFees.GetFormatedValue(true), Code };

            SavingInstrumentFeeEvent sife = new SavingInstrumentFeeEvent
            {
                Amount = 0m,
                Cancelable = true,
                Date = savingsEvent.Date,
                Description = string.Format(desc, items),
                Fee = InstrumentFees.HasValue ? InstrumentFees : 0m,
        ...
        private List<SavingEvent> GenerateSearchFeeEvent(SavingEvent savingsEvent)
        {
            List<SavingEvent> retval = new List<SavingEvent>();

            const string desc = "Search fee of {0:.00} : {1}";
            object[] items = new object[] { SearchFees.GetFormatedValue(true), Code };

            SavingSearchFeeEvent ssfe = new SavingSearchFeeEvent
            {
                Amount = 0m,
                Cancelable = true,
                Date = savingsEvent.Date,
                Description = string.Format(desc, items),
                Fee = SearchFees.HasValue ? SearchFees : 0m,
                User = savingsEvent.User,
                ProductType = Product.GetType(),
                Branch = Branch,
                Currency = Product.Currency,
        ...
        public SavingUnblockCompulsorySavingsEvent GenerateUnblockCompulsorySavingEvent(User user, bool cancellable)
        {
            int contracId = Id;
        ...
        private List<SavingEvent> ChargeInstrumentFee(SavingEvent savingsEvent)
        {
            List<SavingEvent> retval = new List<SavingEvent>();

            List<SavingEvent> list = GenerateInstrumentFeeEvent(savingsEvent);
            if (list != null)
            {
                if (list[0].Fee > 0) retval.Add(list[0]);
                if (list.Count > 1 && list[1].Fee > 0) retval.Add(list[1]);
        ...
        private List<SavingEvent> ChargeSearchFee(SavingEvent savingsEvent)
        {
            List<SavingEvent> retval = new List<SavingEvent>();
            List<SavingEvent> list = GenerateSearchFeeEvent(savingsEvent);
            if (list != null)
            {
                if (list[0].Fee > 0) retval.Add(list[0]);
                if (list.Count > 1 && list[1].Fee > 0) retval.Add(list[1]);
        ...
        private List<SavingEvent> ChargeDutyStampFee(SavingEvent savingsEvent)
        {
            List<SavingEvent> retval = new List<SavingEvent>();
            List<SavingEvent> list = GenerateDutyStampFeeEvent(savingsEvent);
            if (list != null)
            {
                if (list[0].Fee > 0) retval.Add(list[0]);
                if (list.Count > 1 && list[1].Fee > 0) retval.Add(list[1]);
