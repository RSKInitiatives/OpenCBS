...
namespace OpenCBS.Extensions
{
    ...
    public class ContractCodeGenerator : IContractCodeGenerator
    {
        public string GenerateContractCode(IClient client, Loan loan, SqlTransaction transaction)
        {
            ...
            string pBranchCode = loan.BranchCode.Substring(0, Math.Min(loan.BranchCode.Length, 2)).ToUpper();
            string clientCode = client.Id.ToString().PadLeft(4, '0');
            string loanCount = (client.LoanCycle + 1).ToString(CultureInfo.InvariantCulture).PadLeft(2, '0');
            string productCode = loan.Product.Code.Replace(" ", "");
