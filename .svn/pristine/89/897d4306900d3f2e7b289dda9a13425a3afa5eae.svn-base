...
namespace OpenCBS.Services
{
    ...
    public class PicturesServices : MarshalByRefObject
    {
        ...
        public int AddPicture(string pGroup, int pId, int photoSubId, string pFileName, string pName)
        {
            ...
            int newPicrtureSubId = _manager.AddPicture(pGroup, pId, photoSubId, bitmapMStream.ToArray(), 
                thumbnailMStream.ToArray(), pName, "/dashboard/cbs/services/attachments/" + pName + Path.GetExtension(pFileName));
            bitmapMStream.Close();
        ...
        public void UpdatePicture(string group, int personID, int photoSubId, string fileName)
        {
            ...
            _manager.UpdatePicture(group, personID, photoSubId, fileName, resizedImageStream.ToArray(), 
                thumbnailImageStream.ToArray(), "/dashboard/cbs/services/attachments/" + fileName + Path.GetExtension(fileName));
        }        
